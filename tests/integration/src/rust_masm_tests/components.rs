...
use midenc_frontend_wasm::{sanitize_name, ImportMetadata, WasmTranslationConfig};
...
fn wcm_import() {
    ...
    let function_id = *import_info
        .imported(&Ident::from(sanitize_name("miden:add-package/add-interface@1.0.0").as_str()))
        .unwrap()
        .iter()
        .collect::<Vec<_>>()
        .first()
