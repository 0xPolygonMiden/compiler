...
/// Builds a generated note project whose `#[note]` type is declared in `src/lib.rs`.
fn note_project(name: &str, lib_rs: &str) -> crate::cargo_proj::Project {
    let sdk_path = sdk_crate_path();
    let namespace = component_namespace(name);
    let component_package = format!("miden:{}", name.replace('_', "-"));
    let miden_project_toml = format!(
...
"#;
    ...
    let cargo_proj = note_project("note_script_requires_a_note_impl", lib_rs);
    let output = cargo_check_miden_target(&cargo_proj);
    assert!(
        !output.status.success(),
...
"#;
    ...
    let cargo_proj =
        note_project("note_script_account_param_requires_account_wrapper_type", lib_rs);
    let output = cargo_check_miden_target(&cargo_proj);
    assert!(
        !output.status.success(),
...
#[test]
fn note_constructor_with_the_dyncall_prefix_is_rejected() {
    // A note exporting `dyncall-…` compiles fine on its own and only breaks its consumers, where
    // the frontend dispatches the import dynamically instead of linking it; reject it at the
    // producer, where the name is written.
    let lib_rs = r#"#![no_std]
#![feature(alloc_error_handler)]

use miden::{note, note_constructor, note_script, AccountId, Word};
...
"#;
    ...
    let cargo_proj = note_project("note_constructor_reserved_dyncall_prefix", lib_rs);
    let output = cargo_check_miden_target(&cargo_proj);
...
fn component_storage_stored_procedure_rejects_unsafe_fn() {
    // The generated `call` is a safe method, so an `unsafe fn` signature would promise a contract
    // the expansion never surfaces to the caller.
    let lib_rs = r#"#![no_std]
#![feature(alloc_error_handler)]
...
#[test]
fn component_storage_stored_procedure_rejects_type_override() {
    // The component metadata would advertise the overridden schema type while the generated call
    // keeps reading a four-felt procedure root out of the slot.
    let lib_rs = r#"#![no_std]
#![feature(alloc_error_handler)]
...
struct TestComponentStorage {
    #[storage(type = "u32")]
    hook: StorageValue<StoredProcedure<fn(amount: Felt)>>,
}
"#;

    let cargo_proj = account_component_project(
        "component_storage_stored_procedure_rejects_type_override",
        lib_rs,
    );
    let output = cargo_check_miden_target(&cargo_proj);
    assert!(!output.status.success(), "expected a `type` override to be rejected");
    let stderr = String::from_utf8_lossy(&output.stderr);

    assert!(
        stderr.contains("is fixed (a word holding the procedure root)"),
        "unexpected stderr: {stderr}"
    );
}
