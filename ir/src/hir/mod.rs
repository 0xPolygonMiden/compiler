...
impl Module {
    ...
    pub fn declare_function(&mut self, signature: Signature) -> FuncRef {
        ...
        // Think of a parsing of a function call trigerring [`DataFlowGraph::register_callee`] to a not yet declared
        // function in this module populating the `Module::names` and `Module::signatures` maps.
        if let Some(f) = names.get(&signature.name).copied() {
            if let Some(sig) = signatures.get(f) {
                if sig == &signature {
                    return f;
                }
            }
