...
use crate::{ExternalFunction, FunctionIdent, Ident, Immediate, Opcode, Type};
...
pub(super) type GlobalVariablesById =
    FxHashMap<crate::GlobalVariable, Span<crate::GlobalVariableData>>;
pub(super) type ImportsById = FxHashMap<FunctionIdent, Span<crate::ExternalFunction>>;
pub(super) type BlocksById = FxHashMap<crate::Block, Block>;
pub(super) type ValuesById = BTreeMap<crate::Value, Span<crate::ValueData>>;
...
impl Module {
    ...
    pub fn try_into_hir(
        ...
        let mut module = if self.is_kernel {
            crate::Module::new_kernel(self.name)
        } else {
            crate::Module::new(self.name)
        };

        let mut is_valid = true;

        // Validate constants
        let (constants_by_id, is_constants_valid) = self.take_and_validate_constants(diagnostics);
        is_valid &= is_constants_valid;

        // Validate globals
        let (globals_by_id, is_global_vars_valid) =
            self.take_and_validate_globals(&constants_by_id, diagnostics);
        ...
        // Validate imports
        let (imports_by_id, is_externals_valid) = self.take_and_validate_imports(diagnostics);
        is_valid &= is_externals_valid;

        // Validate functions
        let mut functions_by_id = FxHashMap::<Ident, SourceSpan>::default();
        let mut worklist = Vec::with_capacity(self.functions.len());
        ...
        for mut function in worklist.into_iter() {
            ...
            is_valid &= function.is_declaration_valid(diagnostics);
            let entry = function.blocks[0].id;
            let mut blocks_by_id = match function.populate_block_map(diagnostics) {
                Ok(blocks) => blocks,
                Err(blocks) => {
                    is_valid = false;
                    blocks
