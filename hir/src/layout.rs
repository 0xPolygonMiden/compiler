...
use intrusive_collections::{
    intrusive_adapter,
    linked_list::{Cursor, CursorMut, LinkedList},
...
impl<K: EntityRef, V> ArenaMap<K, V> {
    ...
    pub fn contains(&self, key: K) -> bool {
        self.keys.get(key.index()).map(|item| item.is_some()).unwrap_or(false)
    }
...
impl<K: EntityRef, V> OrderedArenaMap<K, V> {
    ...
    pub fn remove(&mut self, key: K) {
        if let Some(value) = self.map.get(key) {
            assert!(value.link.is_linked(), "cannot remove a value that is not linked");
    ...
    ///
    /// The iterator is double-ended, so can be used to traverse the map front-to-back, or
    /// back-to-front
