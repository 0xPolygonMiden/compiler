...
use crate::{
    fmt,
    fmt::{FormatFn, Formatter},
    writer::{self, Writer},
};

/// The default name for the environment variable to read filters from.
pub const DEFAULT_FILTER_ENV: &str = "MIDENC_TRACE";

/// The default name for the environment variable to read kv filters from.
pub const DEFAULT_KV_FILTER_ENV: &str = "MIDENC_TRACE_FILTER";

/// The default name for the environment variable to read style preferences from.
pub const DEFAULT_WRITE_STYLE_ENV: &str = "MIDENC_TRACE_STYLE";
...
impl Builder {
    ...
    /// ```
    /// use log::LevelFilter;
    /// use midenc_log::{Builder, WriteStyle};
    ///
        ...
        E: Into<Env<'a>>,
    {
        ...
        if let Some(s) = env.get_kv_filter() {
            self.parse_kv_filters(&s);
        }
    ...
    /// `midenc_log` formatter takes advantage of this.
    ///
    ...
    /// Parses the directives string in the same form as the `MIDENC_TRACE`
    ...
    /// Parses a kv filter string from the `MIDENC_TRACE_FILTER` environment variable.
    ///
    /// See the module documentation for more details.
    pub fn parse_kv_filters(&mut self, filters: &str) -> &mut Self {
        for filter in filters.split(',').map(|f| f.trim()) {
            let (filter, negated) =
                filter.strip_prefix('-').map(|f| (f, true)).unwrap_or((filter, false));
            let (key, value) = filter
                .split_once(':')
                .expect("invalid kv filter spec: expected format is `key:value[,key2:value2,..]`");
            self.filter.filter_key_value(key, value, negated);
        }
        self
    }
    ...
    /// Parses whether or not to write styles in the same form as the `MIDENC_LOG_STYLE`
    /// environment variable.
    ///
    /// See the module documentation for more details.
...
///
/// - `MIDENC_TRACE`: the level filter
/// - `MIDENC_TRACE_FILTER`: a key-value filter to apply
/// - `MIDENC_TRACE_STYLE`: whether or not to print styles with records.
///
...
impl<'a> Env<'a> {
    ...
    fn get_kv_filter(&self) -> Option<String> {
        self.kv_filter.get()
    }
...
impl Default for Env<'_> {
    fn default() -> Self {
        Env {
            ...
            kv_filter: Var::new(DEFAULT_KV_FILTER_ENV),
...
/// library has already initialized a global logger.
pub fn init() {
    try_init().expect("midenc_log::init should not be called after logger initialized");
}
...
/// let env = Env::new().filter("MY_LOG").write_style("MY_LOG_STYLE");
///
/// midenc_log::try_init_from_env(env)?;
///
/// Ok(())
/// # }
/// # run().unwrap();
    ...
    E: Into<Env<'a>>,
{
    try_init_from_env(env)
        .expect("midenc_log::init_from_env should not be called after logger initialized");
}
...
mod tests {
    ...
    fn env_get_filter_reads_from_default_if_var_not_set() {
        ...
        let env = Env::new()
            .filter_or("env_get_filter_reads_from_default_if_var_not_set", "from default");
    ...
    fn builder_parse_env_overrides_existing_filters() {
        unsafe {
            env::set_var("builder_parse_default_env_overrides_existing_filters", "debug");
        }
