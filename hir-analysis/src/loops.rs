...
mod tests {
    ...
    fn nested_loops_variant1_detection() {
        ...
        assert!(loop_analysis.is_in_loop(block0, loops[0]));
        assert!(!loop_analysis.is_in_loop(block0, loops[1]));
        assert!(loop_analysis.is_in_loop(block1, loops[1]));
        assert!(loop_analysis.is_in_loop(block1, loops[0]));
        assert!(loop_analysis.is_in_loop(block2, loops[1]));
        assert!(loop_analysis.is_in_loop(block2, loops[0]));
        assert!(loop_analysis.is_in_loop(block3, loops[0]));
        assert!(!loop_analysis.is_in_loop(block0, loops[1]));
    ...
    fn nested_loops_variant2_detection() {
        ...
        assert!(!loop_analysis.is_in_loop(block2, loops[1]));
        assert!(loop_analysis.is_in_loop(block3, loops[0]));
        assert!(!loop_analysis.is_in_loop(block3, loops[1]));
        assert!(loop_analysis.is_in_loop(block4, loops[0]));
        assert!(loop_analysis.is_in_loop(block4, loops[1]));
        assert!(loop_analysis.is_in_loop(block5, loops[0]));
        assert!(loop_analysis.is_in_loop(block5, loops[1]));
        assert!(loop_analysis.is_in_loop(block6, loops[0]));
        assert!(!loop_analysis.is_in_loop(block6, loops[1]));
        assert!(domtree.dominates(block4, block6, &function.dfg));
        assert!(!loop_analysis.is_in_loop(exit, loops[0]));
        assert!(!loop_analysis.is_in_loop(exit, loops[1]));
    ...
    fn complex_loop_detection() {
        ...
        assert_eq!(loop_analysis.loop_parent(loops[0]), None);
        assert!(loop_analysis.is_in_loop(block1, loops[0]));
        assert!(loop_analysis.is_in_loop(block2, loops[2]));
        assert!(loop_analysis.is_in_loop(block3, loops[2]));
        assert!(loop_analysis.is_in_loop(block4, loops[1]));
        assert!(loop_analysis.is_in_loop(block5, loops[1]));
        assert!(loop_analysis.is_in_loop(block6, loops[0]));
